package org.jboss.optaplanner.controller.databaseOp;
...
import org.jboss.optaplanner.controller.definition.OrganizationDef;
import org.jboss.optaplanner.controller.definition.TaskDef;
import org.jboss.optaplanner.controller.definition.UserDef;
...
public class Operation {
	...
	*/	
	...
	{   
		Query query = eManager.createQuery("select idUser from User where username='"+username+"'");
		Object idUser = query.getSingleResult();
		
		User usertab = eManager.getReference(User.class,Long.parseLong(idUser.toString()));
		
		query = eManager.createQuery("select organization from User where username='" + username +"'");
	...
	{
		Query q = eManager.createQuery("SELECT  u.idUser,u.username, u.password,u.role, o.nameOfOrganization,u.email FROM User u,Organization o where u.organization=o.idOrganization");
	 ...
	 */
 	...
 	public boolean validateUsername(String username)
 	{
 		...
 		Query q = eManager.createQuery("SELECT username FROM User where username='" +username + "'");
     ...
     */
 	...
 	{
 		Query q = eManager.createQuery("select t.id,t.name,t.status,t.progress,t.eta,t.pub, u.username,t.xmlFile from Task t,User u where t.user = u.idUser");
 	...
 	public List<TaskDef>  selectTaskByUserWithOrganization(String username)
 	{
 		...
 		User user = eManager.getReference(User.class,Long.parseLong(result.toString()));
 		
 		Organization test = (Organization)user.getOrganization();
 		
 		
 		
		Query query2 = eManager.createQuery("select t.id,t.name,t.status,t.progress,t.eta,t.pub, u.username,t.xmlFile from Task t,User u where t.user = u.idUser and t.organization=" + test.getIdOrganization());
	...
	public void changeOrganizationForUser(long id,String org)
	{
	      ...
	      entr.begin();
	      User user = eManager.find(User.class, id);
	      Query query = eManager.createQuery("select idOrganization from Organization where nameOfOrganization='"+ org +"'");
	      Object result = query.getSingleResult();
	...
	{	
		Query query = eManager.createQuery("select idOrganization,nameOfOrganization  from Organization");
...
catch(Exception ex)
{
	...
	public long getIdOrganization(String organization)
	{
		...
		Query q = eManager.createQuery("select idOrganization from Organization where nameOfOrganization='" + organization +"'");
	...
	public long getIdUser(String username)
	{
		...
		Query q = eManager.createQuery("select idUser from User where username='" + username +"'");
	...
	{
		Query q = eManager.createQuery("select username from User where idUser=" + String.valueOf(id));
		Object user= q.getSingleResult();
